...
print(df['acc_norm'].values)

df['acc_norm_sg'] = savitzky_golay(df['acc_norm'].values, 51, 3) #scipy.signal.medfilt(df['acc_norm'].values, 5) #savitzky_golay(df['acc_norm'].values, 51, 3)
plt.figure()
df.iloc[:, -1].plot()
...
for t in range(1, len(df)):
    g_B_s.append(mu * g_B_s[t - 1] + (1 - mu) * df['acc_norm'].values[t])
